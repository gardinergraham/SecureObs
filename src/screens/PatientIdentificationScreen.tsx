...
import type { Patient, PatientIdentificationProfile, StaffMember, Ward } from "../types/domain";
...
export function PatientIdentificationScreen({
  ...
  const printItem = async (type: PatientTagType) => {
    ...
    const html = type === "room" ? roomSignHtml(patient, ward, qrSvg) : personalBadgeHtml(patient, ward, profile, qrSvg);
  ...
  const toggle = (key: "showPhoto" | "showDateOfBirth" | "showHospitalNumber" | "showWardAndRoom") =>
      ...
      <View style={styles.card}>
        ...
        <View style={styles.badgeRow}>
          ...
          <View style={styles.preview}>
            <Text style={styles.previewBrand}>SecureObs patient identification</Text>
            <Text style={styles.previewName}>{patient.firstName} {patient.surname}</Text>
            {profile.showDateOfBirth ? <Text style={styles.body}>DOB: {formatDateOfBirth(patient.dateOfBirth)}</Text> : null}
            {profile.showHospitalNumber ? <Text style={styles.body}>Hospital no: {patient.hospitalNumber}</Text> : null}
            {profile.showWardAndRoom ? <Text style={styles.body}>{ward?.name ?? "Ward"} · Room {patient.roomNumber}</Text> : null}
          </View>
...
function personalBadgeHtml(patient: Patient, ward: Ward | undefined, profile: PatientIdentificationProfile, qrSvg: string) {
  const photo = profile.showPhoto && profile.photoDataUri ? `<img class="photo" src="${profile.photoDataUri}">` : "";
  return documentHtml(`<div class="item"><div class="brand">SecureObs patient identification</div><div class="layout">${photo}<div><div class="name">${escapeHtml(patient.firstName)} ${escapeHtml(patient.surname)}</div><div class="details">${profile.showDateOfBirth ? `DOB: ${escapeHtml(formatDateOfBirth(patient.dateOfBirth))}<br>` : ""}${profile.showHospitalNumber ? `Hospital no: ${escapeHtml(patient.hospitalNumber)}<br>` : ""}${profile.showWardAndRoom ? `${escapeHtml(ward?.name ?? "Ward")} · Room ${patient.roomNumber}` : ""}</div></div></div><div class="qr">${qrSvg}</div><div class="notice">If found, return this badge to the issuing care provider. The QR code contains no visible clinical information.</div></div>`);
}
...
const styles = StyleSheet.create({
  ...
  preview: { backgroundColor: "#f8fbfc", borderColor: "#9bc9d2", borderRadius: 8, borderWidth: 1, flex: 1, padding: 16 }, previewBrand: { color: "#07879b", fontWeight: "900" }, previewName: { color: "#082456", fontSize: 24, fontWeight: "900", marginVertical: 9 }, displayOptions: { flex: 1, gap: 7 },
