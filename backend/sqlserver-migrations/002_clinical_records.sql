...
begin
  create table dbo.medication_prescriptions (
    ...
    prescription_type nvarchar(50) not null default 'regular',
    prn_indication nvarchar(max) null,
    depot_interval_days int null,
...
if not exists (select 1 from sys.indexes where name = 'ix_food_fluid_organisation_patient' and object_id = object_id('dbo.food_fluid_entries'))
begin
  create index ix_food_fluid_organisation_patient on dbo.food_fluid_entries(organisation_id, patient_id, recorded_at desc);
end;
